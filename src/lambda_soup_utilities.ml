...
let get_first_item_of_all_unordered_lists contents : string list =
  ...
  $$ "ul"
  |> to_list
  |> List.map ~f:(fun ul -> select "li" ul |> to_list |> take_first |> texts |> String.concat ~sep:"" |> String.strip)
;;
...
let get_first_item_of_second_unordered_list contents : string =
  ...
  parse contents
  $$ "ul"
  |> to_list |> take_last |> take_first
  |> select "li" |> to_list |> take_first |> texts |> String.concat ~sep:"" |> String.strip
;;
